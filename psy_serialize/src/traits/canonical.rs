...
mod tests {
    ...
    impl FallbackPsySerializeCanonical for ExampleFallbackStruct {
        ...
        fn fallback_pio_write_to_io<W: Write>(&self, writer: &mut W) -> anyhow::Result<()> {
            ...
            writer.write_all(&self.a).map_err(|e| anyhow::anyhow!("{:?}", e))?;
            writer.write_all(&self.b.to_le_bytes()).map_err(|e| anyhow::anyhow!("{:?}", e))?;
            Ok(())
        }
        ...
        fn fallback_pio_read_from_io<R: Read>(reader: &mut R) -> anyhow::Result<Self> {
            ...
            reader.read_exact(&mut a).map_err(|e| anyhow::anyhow!("{:?}", e))?;
            let mut b_data = [0u8; 4];
            reader.read_exact(&mut b_data).map_err(|e| anyhow::anyhow!("{:?}", e))?;
